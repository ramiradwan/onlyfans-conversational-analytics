...
Production acceptance requires an authenticated, confidentiality- and integrity-protected Agent-to-Brain session before any Full-mode user data, keys, auth tickets, or rotation material cross that boundary. Preview must remain standalone. Native-permission and session acceptance must cover the exact ZIP on Chrome 132 and the current browser, including hostile port ownership, deadlines, cancellation, bounded frames and responses, replay, and downgrade refusal. Scoped live traversal needs an authorized account, bounded scope, observed counts, deduplication and reconstruction evidence. Synthetic fixtures and upstream signer evidence do not establish consumer live-history completeness.

Browser-trusted HTTPS/WSS is not required by this session-security requirement. [ADR 0024](../../docs/adr/0024-authenticated-companion-sessions.md) defines the application-layer encryption investigation and trust-bootstrap review gate. The current production HTTPS/WSS enforcement remains unchanged until that review authorizes an implementation. Bridge/browser transport security is a separate architecture review.
