...
Browser-trusted HTTPS/WSS is not required by this session-security requirement. [ADR 0024](../../docs/adr/0024-authenticated-companion-sessions.md) defines the Noise KK session and the local pairing that authenticates Brain. Full-mode boundary operations use the authenticated session over the loopback WebSocket, including configuration and storage-key release. Bridge/browser transport security is a separate architecture review.
