...
const agentToBrainTypes = new Set(['agent.hello', 'agent.heartbeat', 'ingest.snapshot', 'ingest.delta', 'presence.observed', 'config.applied']);
const brainToAgentTypes = new Set(['agent.session', 'sync.required', 'ingest.ack', 'ingest.rejected', 'protocol.error', 'config.available']);
...
const inactiveActionPolicy = object({
  allowed_actions: array(() => {}, 0, 0),
...
const configDocumentResponse = object({
  ...
  command_policy: inactiveActionPolicy,
