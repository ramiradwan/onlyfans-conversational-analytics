...
export class HistoryAcquisitionCoordinator {
  ...
  async #run(authorization, signal, attempt) {
    throwIfAborted(signal);
  ...
  async #readSigner(request, authorization, signal, attempt, job = null) {
  ...
  async #startGeneration({ policy, session }, signal, attempt) {
  ...
  async #readInventoryPage(job, authorization, signal, attempt) {
    ...
    try {
      ...
      const data = assertPage(await this.#readSigner({
  ...
  async #readConversationPage(job, authorization, signal, attempt) {
    const { policy } = authorization;
    try {
      job = job.lease_token === this.leaseToken
        ? job
