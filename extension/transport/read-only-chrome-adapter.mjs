import { INGESTION_STORES } from './read-only-durable-outbox.mjs';
import {
...
/** Owns the deliberately small installation-global chrome.* surface. */
...
export function createChromeAdapter(
  ...
  const partitionStorage = (databaseName) => createReadOnlyIndexedDbIngestionStorage(indexedDb, {
    databaseName,
    cryptoApi,
  });
