...
class GraphBuilder:  
    """Converts enriched conversation data into graph vertices and edges following the LPG schema."""  
  
    def __init__(self, creator_id: str):  
        self.creator_id = normalize_str(creator_id)  
  
    def build_fan_vertex(self, fan_id: str, join_date: datetime = None,  
                         demographics: Dict[str, Any] = None, sentiment: float = None) -> Fan:  
        return Fan(  
    ...
    def build_creator_vertex(self, niche: str = None, style_profile: Dict[str, Any] = None) -> Creator:  
        return Creator(  
    ...
    def build_conversation_vertex(self, conversation_id: str, start_date: datetime, end_date: datetime,  
                                  message_count: int, avg_response_time: float, turns: int,  
                                  silence_pct: float) -> ConversationNode:  
    ...
    def _ensure_models(self, items: List[Any], model_cls) -> List[Any]:  
        result = []  
        for item in items:  
    ...
    def build_edges(self, fan: Fan, conversation: ConversationNode,  
                    topics: List[Topic], actions: List[EngagementAction],  
                    outcomes: List[InteractionOutcome]) -> List[GraphEdge]:  
        edges: List[GraphEdge] = []  
  
        edges.append(GraphEdge(from_id=fan.fanId, to_id=conversation.conversationId,  
                               label="HAS_CONVERSATION", properties={"attendance": "Participated"}))  
  
        for topic in topics:  
            edges.append(GraphEdge(from_id=conversation.conversationId, to_id=topic.topicId,  
                                   label="DISCUSS_TOPIC", properties={}))  
  
        for action in actions:  
            edges.append(GraphEdge(from_id=conversation.conversationId, to_id=action.actionId,  
                                   label="USES_ENGAGEMENT", properties={}))  
  
        for action in actions:  
            for topic in topics:  
                edges.append(GraphEdge(from_id=action.actionId, to_id=topic.topicId,  
                                       label="TARGETS_TOPIC", properties={"rationale": "Placeholder"}))  
  
        for outcome in outcomes:  
            edges.append(GraphEdge(from_id=conversation.conversationId, to_id=outcome.outcomeId,  
                                   label="RESULTS_IN_OUTCOME", properties={"changeFromPrevious": None}))  
  
        return edges  
  
    def build_graph(self, enriched_conv: dict, fan_id: str) -> Dict[str, List]:  
        logger.info(f"Building graph for conversation {enriched_conv.get('conversationId')} and fan {fan_id}")  
  
        fan_vertex = self.build_fan_vertex(fan_id, sentiment=enriched_conv.get("sentiment"))  
        creator_vertex = self.build_creator_vertex()  
  
        # Determine start/end dates safely  
        start_date = enriched_conv.get("startDate") or None  
        end_date = enriched_conv.get("endDate") or None  
  
        # Try extracting from messages if not provided  
        msgs = enriched_conv.get("messages") or []  
        if not start_date and msgs:  
            start_date = msgs[0].created_at if hasattr(msgs[0], "created_at") else msgs[0].get("created_at")  
        if not end_date and msgs:  
            end_date = msgs[-1].created_at if hasattr(msgs[-1], "created_at") else msgs[-1].get("created_at")  
