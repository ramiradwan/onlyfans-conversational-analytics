...
| `brain-transport` | Red | Authoritative | Brain network admission, WebSocket connection lifecycle, protocol framing, and session dispatch to canonical persistence (`app/transport/**`, `app/api/endpoints/transport_ws.py`, `app/api/endpoints/companion_pairing.py`, `app/api/endpoints/companion_session.py`). |
...
| `canonical-persistence` | Red | Authoritative | Core authoritative canonical conversation storage, SQLite database lifecycle and SQLCipher runtime qualification, schema migrations, and irreversible deletion closure (`app/persistence/history.py`, `app/persistence/database.py`, `app/persistence/migrations/**`, `app/persistence/deletion_operations.py`, `app/persistence/auth.py`, `app/persistence/companion_pairing.py`, `app/persistence/private_files.py`, `app/persistence/sqlite_api.py`, `app/persistence/sqlcipher_runtime.py`). |
...
| `security-trust` | Red | Authoritative | Side authority governing local authentication, device-bound keys, activation gate, grant verification, and capability permits (`app/security/**`, `app/api/security.py`, `native/companion-snow/**`). |
