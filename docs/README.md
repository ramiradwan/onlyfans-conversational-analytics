# Documentation

Use these pages to find the instructions or reference material for a specific task.

## Use the product

- [Verify a release](verify-release.md)
- [Install on Windows](install-windows.md)

## Develop

- [Contribute to the project](../CONTRIBUTING.md)
- [Test changes](testing.md)

## Release and acceptance

- [Build Windows release artifacts](build-windows.md)
- [Run product qualification](qualification.md)
- [Run Windows acceptance](installation-and-acceptance.md)

## Understand the system

- [Architecture decisions](adr/README.md)
- [Communication overview](../communication-spec.md)
- [Brain](../app/README.md)
- [Agent](../extension/README.md)
- [Bridge](../frontend/README.md)
- [Bridge frontend design](../frontend/frontend-design-spec.md)

## Write documentation

- [Documentation style](style.md)
