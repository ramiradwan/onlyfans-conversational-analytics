...
A cache miss performs the existing exact content scan under the caller's budget. Supplied transaction connections always scan their own data and never share cached identities. The canonical digest format, activation witness, deletion rules, and analysis admission are unchanged. Source tokens cannot make an unverified generation visible.
