...
- [ADR 0014: Define the local authorization foundation](0014-local-authorization-foundation.md) — Proposed immutable runtime policy, purpose-separated capability authority, local execution invariants, restart-safe jobs, pinned trust, and canonical knowledge contributions.
- [ADR 0015: Define the non-expiring capability licence profile](0015-non-expiring-capability-licence-profile.md) — Proposed repeated and incremental local execution under an installation- and seat-bound capability object with no per-execution authorization-state change.
- [ADR 0016: Define the single-execution permit profile](0016-single-execution-permit-profile.md) — Proposed atomic reservation, durable job recovery, contribution-and-spend commitment, and completed-result retrieval for one-execution authority.
- [ADR 0017: Select packaged boot mode by runtime configuration](0017-configuration-selected-boot-modes.md) — Proposed configuration-file presence selector for exclusive provisioning and configured import graphs with a fresh-process completion restart.
