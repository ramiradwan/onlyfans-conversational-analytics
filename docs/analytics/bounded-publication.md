<!-- CODE-VERIFY: Check generation_reference.py, graph_verification.py, shared_graph.py, validation_receipt.py, graph_row_encoding.py, projection_encoding.py, projection_verification.py, retention_store.py, pipeline.py, both SQLite stores, conversation_reuse.py, and qualify_continuous_analytics.py before editing behavior claims. -->
...
Cold graph verification reads ordered rows, validates their types and properties, and computes the canonical digest from their actual contents. An indexed join checks edge endpoints within the candidate. Account scope and persisted counts are checked independently. Matching two supplied digest strings is not sufficient. [ADR 0038](../adr/0038-verified-graph-segment-reuse.md) permits an incremental staging validation to reuse a process-local proof for unchanged immutable segments from the exact active predecessor while validating changed segments from persisted rows. Missing or invalid proof uses the complete row path. The final activation gate still requires either an unchanged live receipt under ADR 0036 or another complete verification.

When graph objects are not requested, the full verifier checks stored columns directly using the same identity and closed-property rules as the public models. It normalizes timestamps and encodes the same canonical JSON without constructing temporary graph models. Non-integer or negative stored edge sequences are rejected. That scan also records verified segment identities, digests, counts and categories for possible ADR 0038 reuse. Full artifact reads materialize the validated graph explicitly. Database statement progress and row-level checks retain cancellation and deadline handling.
