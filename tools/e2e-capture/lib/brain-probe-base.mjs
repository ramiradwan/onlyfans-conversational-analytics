...
function requestBrain(pathname, { body = null, headers = {}, method = 'GET' } = {}) {
...
export async function completeBrowserWebAuthnCeremony(page) {
  ...
  const result = await page.evaluate(async () => {
    ...
    const post = async (pathValue, body) => {
      const response = await fetch(`/api/v1/webauthn${pathValue}`, {
...
function parsePairingDocument(response, operation) {
  if (response.status < 200 || response.status >= 300) {
...
export async function openCompanionPairing(context) {
  const cookie = await browserSessionCookieHeader(context);
  const { config, csrfToken } = await servedBootstrap(context);
  const body = JSON.stringify({ creator_account_id: config.CREATOR_ID });
  const response = await requestBrain('/api/v1/companion/pairings', {
    body,
    headers: {
  ...
  const document = parsePairingDocument(response, 'Companion pairing open');
  if (
    typeof document?.pairing_id !== 'string'
    || typeof document?.version !== 'number'
    || document.creator_account_id !== config.CREATOR_ID
  ) throw new Error('Brain returned an invalid companion pairing window.');
  return { ...document, creatorAccountId: config.CREATOR_ID, extensionId: config.EXTENSION_ID };
}

export async function readCompanionPairingStatus(context, pairingId) {
  const cookie = await browserSessionCookieHeader(context);
  const response = await requestBrain(`/api/v1/companion/pairings/${encodeURIComponent(pairingId)}`, {
    headers: {
      Accept: 'application/json',
      Cookie: cookie,
      Origin: BRAIN_ORIGIN,
  ...
  return parsePairingDocument(response, 'Companion pairing status');
}

export async function confirmCompanionPairing(context, pairingId, version) {
  const cookie = await browserSessionCookieHeader(context);
  const { csrfToken } = await servedBootstrap(context);
  ...
  return parsePairingDocument(response, 'Companion pairing confirmation');
}
