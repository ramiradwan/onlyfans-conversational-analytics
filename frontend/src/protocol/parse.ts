...
const messagePayloadValidators: Record<string, Validator> = {
  ...
  'bridge.hello': object({
    ...
    requested_creator_account_id: nonEmptyString,
    capabilities: array(literal('state.snapshot', 'state.delta', 'presence.state'), 1),
...
const brainToBridgeTypes = new Set(['bridge.session', 'state.snapshot', 'state.delta', 'presence.state', 'agent.state', 'system.state', 'protocol.error']);
...
function parseDirectional<T>(value: unknown, allowedTypes: ReadonlySet<string>): T {
  ...
  if (typeof type !== 'string' || !allowedTypes.has(type) || !(type in messagePayloadValidators)) {
    throw new ProtocolValidationError('$.type', 'message is not allowed in this direction');
  }
  object(
    { type: literal(type), protocol_version: literal('1'), message_id: uuid, payload: messagePayloadValidators[type] },
...
export const parseAgentToBrainMessage = (value: unknown): AgentToBrainMessage => parseDirectional(value, agentToBrainTypes);
export const parseBrainToAgentMessage = (value: unknown): BrainToAgentMessage => parseDirectional(value, brainToAgentTypes);
export const parseBridgeToBrainMessage = (value: unknown): BridgeToBrainMessage => parseDirectional(value, bridgeToBrainTypes);
export const parseBrainToBridgeMessage = (value: unknown): BrainToBridgeMessage => parseDirectional(value, brainToBridgeTypes);

export const isAgentToBrainMessage = (value: unknown): value is AgentToBrainMessage => isParsedBy(parseAgentToBrainMessage, value);
export const isBrainToAgentMessage = (value: unknown): value is BrainToAgentMessage => isParsedBy(parseBrainToAgentMessage, value);
export const isBridgeToBrainMessage = (value: unknown): value is BridgeToBrainMessage => isParsedBy(parseBridgeToBrainMessage, value);
export const isBrainToBridgeMessage = (value: unknown): value is BrainToBridgeMessage => isParsedBy(parseBrainToBridgeMessage, value);
...
export const isAgentConfigGetRequest = (value: unknown): value is AgentConfigGetRequest => isParsedBy(parseAgentConfigGetRequest, value);
export const isAgentConfigDocumentResponse = (value: unknown): value is AgentConfigDocumentResponse => isParsedBy(parseAgentConfigDocumentResponse, value);
