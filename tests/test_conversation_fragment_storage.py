...
def test_empty_fragment_validation_does_not_touch_artifact():
    from app.analytics.conversation_reuse import iter_validated_fragments

    class Forbidden:
        def __getattribute__(self, name):
            raise AssertionError("empty fragment validation touched the artifact")

    assert list(iter_validated_fragments(Forbidden(), ())) == []
