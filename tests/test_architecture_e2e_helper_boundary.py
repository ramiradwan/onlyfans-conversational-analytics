...
installation key through the real persistence transitions so the capture E2E
can seed WebAuthn grants on a host with no TPM-backed provider.
``provisioning_grant_authority.py``, ``pairing_fixture.py``, and
``brain_entry.py`` are likewise test-only authority seams. They ship with
nothing and depend on ``app``, not the other way round: nothing under ``app``
may reference any of them, or synthetic key/trust behavior could silently reach
a real installation.
"""
...
E2E_HELPER_NAMES = (
    "brain_entry",
    "pairing_fixture",
    "provisioning_grant_authority",
    "seed_webauthn_grants",
)
...
def test_guard_rejects_the_pairing_authority_in_an_app_module(tmp_path: Path) -> None:
    path = tmp_path / "app" / "security" / "example.py"
    path.parent.mkdir(parents=True)
    path.write_text(
        "from tools.e2e_capture.helpers.pairing_fixture import SyntheticPairingAuthority\n",
        encoding="utf-8",
    )

    assert _e2e_helper_violations(tmp_path / "app") == [
        "security/example.py: ['imports from tools.e2e_capture.helpers.pairing_fixture']"
    ]
